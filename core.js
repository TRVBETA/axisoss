...
let coreDataState = {
    ...
    momentum: { tasks: [], rituals: [], taskTotals: { active: 0, currentBest: 0, longestBest: 0, totalCurrent: 0, tier: { key: 'none', label: 'NONE', rank: 0 } }, ritualTotals: { active: 0, currentBest: 0, longestBest: 0, totalCurrent: 0, tier: { key: 'none', label: 'NONE', rank: 0 } }, totalCurrent: 0, totalLongest: 0, overallTier: { key: 'none', label: 'NONE', rank: 0 } },
    review: null,
    draftBalanceLabel: '',
    draftBalanceAmount: '',
    draftTodo: '',
...
function formatModeLabel(mode = 'desk') {
    const clean = String(mode || 'desk').trim().toLowerCase();
...
function getMomentumTierClass(tierKey = 'none') {
    ...
    return ['ember', 'spark', 'flare', 'blaze', 'inferno'].includes(clean) ? clean : 'none';
}

function getTaskMomentumEntry(todo = {}) {
    const source = String(todo.task_kind || 'task') === 'ritual'
        ? (coreDataState.momentum?.rituals || [])
        : (coreDataState.momentum?.tasks || []);
    return source.find(item => item.task_id === todo.id || item.title === todo.title) || null;
}
...
function renderMomentumChips() {
    ...
    return `<div class="row flex-wrap" style="gap: 8px;">${tasks.slice(0, 6).map(item => `<span class="badge badge-muted axis-streak-badge axis-streak-${getMomentumTierClass(item.tier?.key)}">${escapeHtml(item.tier?.label || 'STREAK')} • ${item.currentStreak || 0}d</span>`).join('')}</div>`;
}
...
function renderTodoListHTML(items = coreDataState.todos) {
    ...
    return items.slice(0, 24).map(todo => {
        ...
        const streakTier = getMomentumTierClass(momentumEntry?.tier?.key || 'none');
...
function closeClipboardModal() {
    ...
    document.documentElement.style.overflow = coreDataState.taskModalOpen ? 'hidden' : '';
...
async function handleTodoAdd(e) {
    ...
    try {
        ...
        setCoreSyncVisual('quiet');
        document.documentElement.style.overflow = clipboardState.modalOpen ? 'hidden' : '';
