...
export async function fetchCoreData() {
  ...
  const momentum = await fetchRitualMomentum();
  return {
    balance,
    todos: normalized,
    markers,
...
export async function fetchRitualMomentum() {
  try {
    const rows = await supabaseRequest('core_task_events?select=task_id,title_snapshot,event_type,day_key,task_kind_snapshot,created_at&task_kind_snapshot=eq.ritual&order=created_at.desc&limit=1000');
    const byTask = new Map();
    for (const row of rows || []) {
      if (!row.task_id || row.event_type !== 'completed' || !row.day_key) continue;
      if (!byTask.has(row.task_id)) byTask.set(row.task_id, { title: row.title_snapshot || 'Ritual', days: new Set() });
      byTask.get(row.task_id).days.add(row.day_key);
    }
    ...
    for (const [taskId, info] of byTask.entries()) {
      const days = [...info.days].sort();
      let longest = 0;
      let current = 0;
      let prev = null;
      ...
      for (let i = 0; i < 365; i += 1) {
        ...
        const key = new Intl.DateTimeFormat('en-CA', { timeZone: 'Africa/Cairo', year: 'numeric', month: '2-digit', day: '2-digit' }).format(d);
        if (info.days.has(key)) current += 1;
        else break;
      }
      totalCurrent += current;
      totalLongest += Math.max(longest, current);
      result.push({ task_id: taskId, title: info.title, currentStreak: current, longestStreak: Math.max(longest, current) });
    }

    return {
      rituals: result.sort((a, b) => b.currentStreak - a.currentStreak),
      totalCurrent,
      totalLongest
    };
  } catch {
    return { rituals: [], totalCurrent: 0, totalLongest: 0 };
  }
}
