...
function normalizePreparationMode(mode) {
  const clean = String(mode || 'auto').trim().toLowerCase();
  return PREPARATION_MODES.has(clean) ? clean : 'auto';
}
...
export async function writeNutritionLog(text, source = 'axis_web', options = {}) {
  ...
  return { inserted, summary, totals, items, loggedAt: createdAt };
}
...
export async function fetchNutritionSummary(limit = 20) {
  ...
  const todayKey = cairoDayKeyFromDate(new Date());
  const todayRows = (rows || []).filter(row => cairoDayKeyFromDate(new Date(row.logged_at)) === todayKey);
