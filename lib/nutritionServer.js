...
/**
 * Write pre-computed nutrition macros directly, bypassing the parser.
 * Used by the MyFitnessPal / Apple Health ingest path where each item
 * is already a clean (description, quantity, unit, cal, p, c, f) row.
 *
 * `entries` is an array of:
 *   { logged_at, items: [{ name, quantity, unit, calories, protein, carbs, fat }] }
 * The whole `entries` array is treated as one batch per call. Within
 * a batch, each item becomes one row in nutrition_logs.
 */
export async function writeNutritionMacros(entries, source = 'apple_health') {
  if (!Array.isArray(entries) || !entries.length) {
    throw new Error('NO ENTRIES PROVIDED');
  }

  // Flatten entries -> rows. logged_at is the batch key, so rows from
  // the same entry share the same timestamp (grouped in the same batch).
  const payload = [];
  for (const entry of entries) {
    const loggedAt = String(entry.logged_at || new Date().toISOString());
    const items = Array.isArray(entry.items) ? entry.items : [];
    for (const it of items) {
      const description = String(it.name || it.description || 'item').trim().slice(0, 200);
      if (!description) continue;
      payload.push({
        description,
        quantity: Number(it.quantity || 1) || 1,
        unit: String(it.unit || 'serving').trim().slice(0, 40),
        calories: Number(Number(it.calories || 0).toFixed(2)),
        protein: Number(Number(it.protein || 0).toFixed(2)),
        carbs: Number(Number(it.carbs || 0).toFixed(2)),
        fat: Number(Number(it.fat || 0).toFixed(2)),
  ...
  if (!payload.length) throw new Error('NO ITEMS PARSED');

  // Supabase POST supports batch insert. If the body is an array it
  // bulk-inserts all rows in one call.
  const inserted = await supabaseRequest('nutrition_logs', {
    method: 'POST',
    body: payload
  });

  // Recompute score for the most recent entry's date.
  const lastDate = entries[entries.length - 1]?.logged_at || new Date().toISOString();
  await recalcNutritionScoreForToday(lastDate);

  return {
    inserted,
    rowCount: payload.length,
    entryCount: entries.length,
    lastLoggedAt: lastDate
  };
}
